...
st.markdown("### 🚀 OneData 통합 ETL DAG 생성 플랫폼")
st.markdown("---")
...
with col2:
    author = st.text_input("Owner", value="추학태")
with col3:
    email = st.text_input("Email", value="htchoo@sailingstone.io")
with col4:
...
with col5:
    dag_types = [
        ...
        "반복문 적재 (Loop ETL)", 
        "커스텀 라이브러리 실행 (Python)",
        "태블로원본 추출 (Tableau)"  # 💡 추가됨: 태블로 유형 추가
    ]
...
lib_module, lib_func = "", ""
target_datasource = ""  # 💡 추가됨: 태블로 원본명 변수 초기화
...
if dag_type == "표준 ETL (단일 쿼리 적재)":
    ...
    with d_col1:
        ...
        st.caption("LG ONED BQCallProcOperator 전용 커넥션")
    with d_col2:
        proc_name = st.text_input("프로시저명", placeholder="L1_ONED.SP_TRMRKT_GA_CMPGN...")
        st.caption("✨ 호출 시 [dag_id, 파라미터들...] 순서로 자동 세팅됩니다.")
    ...
    if not dag_id:
        ...
        try:
            ...
            rendered_code = template.render(
                ...
                target_datasource=target_datasource,  # 💡 추가됨: 태블로 템플릿 렌더링용 변수 주입
                fail_alert=fail_alert, success_alert=success_alert, auto_tags=auto_tags
            )
            
            st.success(f"✨ [{project_name}] {dag_type} 생성 완료!")
